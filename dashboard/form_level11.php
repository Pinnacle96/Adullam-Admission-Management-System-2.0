...
// ✅ Ensure error reporting for PDO
$pdo->setAttribute(PDO::ATTR_ERRMODE, PDO::ERRMODE_EXCEPTION);

// 🚫 Block access unless logged in as student
...
// ✅ Ensure `applications` record exists
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    $ma_focus      = isset($_POST['ma_focus']) ? trim($_POST['ma_focus']) : null;
    $mode          = trim($_POST['mode']);

    // ⛔ Force mode to "online" for MA Biblical Studies
    if ($program === 'MA' && $ma_focus === 'MA Biblical Studies (OT/NT)') {
        $mode = 'onsite';
    }

    // ❌ Clear ma_focus if program is not MA
    if ($program !== 'MA') {
        $ma_focus = null;
    }
    ...
    // ✅ INSERT or UPDATE application_details
    try {
        $stmt = $pdo->prepare("
            INSERT INTO application_details (
                user_id, gender, dob, program, ma_focus, mode_of_study,
            ...
            ON DUPLICATE KEY UPDATE
                ...
                perm_address = VALUES(perm_address),
...
<body class="bg-gray-100 flex items-center justify-center min-h-screen px-4">
    ...
    <div class="bg-white p-6 sm:p-8 rounded-xl shadow-xl w-full max-w-3xl">
        ...
        <form method="POST" class="space-y-6">
            ...
            <script>
    const programSelect = document.getElementById('program');
    const focusSelect = document.querySelector('select[name="ma_focus"]');
    const modeSelect = document.querySelector('select[name="mode"]');
    const maFocusWrapper = document.getElementById('maFocusWrapper');
    const maWarning = document.getElementById('maWarning');

    programSelect.addEventListener('change', function () {
        const isMA = this.value === 'MA';
        maFocusWrapper.classList.toggle('hidden', !isMA);
    });

    focusSelect.addEventListener('change', function () {
        const selectedFocus = this.value;
        if (selectedFocus.includes('Biblical Studies')) {
            // Show warning & force mode
            maWarning.classList.remove('hidden');
            modeSelect.value = 'onsite';
            modeSelect.setAttribute('disabled', true);
        } else {
            maWarning.classList.add('hidden');
    ...
    document.getElementById('copyAddress').addEventListener('change', function () {
        if (this.checked) {
            document.querySelector('input[name="perm_address"]').value = document.querySelector('input[name="res_address"]').value;
            document.querySelector('input[name="perm_city"]').value = document.querySelector('input[name="res_city"]').value;
            document.querySelector('input[name="perm_state"]').value = document.querySelector('input[name="res_state"]').value;
            document.querySelector('input[name="perm_country"]').value = document.querySelector('input[name="res_country"]').value;
        } else {
            document.querySelectorAll('input[name^="perm_"]').forEach(el => el.value = '');
        }
    });
